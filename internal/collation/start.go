...
func (s *State) Start(anchor lexicon.Key, witnesses []lexicon.Key) (lexicon.Collation, error) {
	...
	v := lexicon.Collation{ID: lexicon.KeyOf("collation", string(anchor)), Anchor: anchor, Witnesses: lexicon.Unique(witnesses), RuleSetID: s.rules.ID, Shards: len(witnesses), Status: "queued"}
	s.collations[v.ID] = v
	return v, nil
}
